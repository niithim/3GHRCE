...
async def process_dataset(api_docs_url: str):
    ...
    csv_filename = os.path.join(csv_folder, f"{dataset_slug}.csv")
...
async def process_direct_csv_dataset(dataset_id: str, dataset_slug: Optional[str] = None):
    ...
    output_file = os.path.join(csv_folder, f"{dataset_slug}.csv")
...
async def process_state_average_dataset():
    ...
    async with httpx.AsyncClient(timeout=30.0) as client:
        try:
            ...
            # Save to CSV
            df.to_csv(os.path.join(csv_folder, f"{dataset_slug}.csv"), index=False)
            logger.info(f"State average data saved to {os.path.join(csv_folder, dataset_slug)}.csv")
